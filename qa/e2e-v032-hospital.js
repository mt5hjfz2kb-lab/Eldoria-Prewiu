...
(async()=>{
 ...
 if(migrated.wood!==before.wood||migrated.stone!==before.stone||migrated.food!==before.food||migrated.bastionLevel!==before.bastion||migrated.troops!==before.troops||migrated.lyra!==before.lyra||migrated.forge!==before.forge||migrated.inventory.map(i=>i.id).sort().join('|')!==before.inventory)throw Error('Old-save migration changed existing progress '+JSON.stringify({before,after:{wood:migrated.wood,stone:migrated.stone,food:migrated.food,bastion:migrated.bastionLevel,troops:migrated.troops,lyra:migrated.lyra,forge:migrated.forge,inventory:migrated.inventory.map(i=>i.id).sort().join('|')}}));

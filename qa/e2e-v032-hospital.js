...
(async()=>{
 ...
 const before=await migration.evaluate(()=>{const x=window.ELDORIA_V023.state();x.wood=4321;x.stone=3210;x.food=2222;x.bastionLevel=9;x.troopRoster={archer:{1:77,2:5,3:0},paladin:{},warlock:{}};x.troops=82;delete x.woundedRoster;delete x.hospitalUnlocked;delete x.hospitalTutorialSeen;delete x.hospitalFirstTreatmentComplete;delete x.pvpCasualties;delete x.marchOperation;localStorage.setItem('eldoria-v022-consistent-loop',JSON.stringify(x));return{x:x.wood,stone:x.stone,food:x.food,bastion:x.bastionLevel,troops:x.troops}});
 ...
 if(migrated.wood!==before.x||migrated.stone!==before.stone||migrated.food!==before.food||migrated.bastionLevel!==before.bastion||migrated.troops!==before.troops)throw Error('Old-save migration changed existing progress '+JSON.stringify({before,migrated}));

...
(async()=>{
 ...
 if(Math.floor(migrated.wood)!==Math.floor(before.wood)||Math.floor(migrated.stone)!==Math.floor(before.stone)||Math.floor(migrated.food)!==Math.floor(before.food)||migrated.bastionLevel!==before.bastion||migrated.troops!==before.troops||migrated.lyra!==before.lyra||migrated.sawmill!==before.sawmill||migrated.camp!==before.camp||migrated.inventory.map(i=>i.id).sort().join('|')!==before.inventory)throw Error('Old-save migration changed existing progress '+JSON.stringify({before,after:{wood:migrated.wood,stone:migrated.stone,food:migrated.food,bastion:migrated.bastionLevel,troops:migrated.troops,lyra:migrated.lyra,sawmill:migrated.sawmill,camp:migrated.camp,inventory:migrated.inventory.map(i=>i.id).sort().join('|')}}));

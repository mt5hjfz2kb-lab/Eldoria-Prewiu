...
(async()=>{
 ...
 await choose('b7-codex');await p.locator('[data-testid="codex-scroll"]').waitFor({state:'visible'});s=await state();if(!s.codexUnlocked||s.codex.length!==1||s.relicSidesRevealed||s.chapterProgress.current!==7)throw Error('Codex first-relic preset incoherent');
  await choose('relicario-v029');await p.locator('[data-testid="relicario"]').waitFor({state:'visible'});s=await state();if(s.view!=='relicario'||((s.codex||[]).length+(s.cardsConsumed||[]).length)<5||!s.relicSidesRevealed)throw Error('Relicario threshold preset incoherent');

...
(async()=>{
 ...
 let d=await open(),s;if(await d.locator('[data-qa-preset]').count()<14)throw Error('QA launcher preset catalog incomplete');
 ...
 await choose('b7-codex');await p.locator('[data-testid="codex-scroll"]').waitFor({state:'visible'});s=await state();if(!s.codexUnlocked||s.codex.length!==1||s.relicSidesRevealed||s.chapterProgress.current!==7)throw Error('Codex first-relic preset incoherent');
  await choose('relicario-v031');await p.locator('[data-testid="relicario"]').waitFor({state:'visible'});s=await state();if(s.view!=='relicario'||(s.relicDiscovered||[]).length<4||!s.relicIndestructibles?.['estandarte-valoria']||!s.relicSidesRevealed)throw Error('Relicario v0.31 preset incoherent');

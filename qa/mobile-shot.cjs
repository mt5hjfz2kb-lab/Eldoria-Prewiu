...
(async()=>{
 ...
 const shot=async(name,state,selector)=>{await p.evaluate(s=>window.ELDORIA_V022_QA.setState(s),state);await p.waitForSelector(selector+':visible');await p.screenshot({path:'qa/'+name+'.png',fullPage:true})};
 ...
 await shot('mobile-review-world',{introSeen:true,view:'world',bastionLevel:9,bastion:9,sawmill:true,barracks:true,granary:true,graniteQuarry:true,forge:true,lyra:true,maelis:true,marchConfigured:true,marchSlots:['aldric','lyra'],trial9Done:false},'#app>.e22 [data-testid="world-map"]');
 await b.close();

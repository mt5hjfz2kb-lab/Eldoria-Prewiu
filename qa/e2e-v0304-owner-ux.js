...
(async()=>{
 ...
 const recruitAction=barracksContext.locator('[data-testid="building-action-barracks"]');await recruitAction.waitFor({state:'visible'});await recruitAction.click({force:true});
 ...
 await set({...await state(),view:'kingdom',bastionLevel:7,codexUnlocked:true,relicarioUnlocked:true,codex:[{id:'ash-sigil',name:'Sello de Ceniza',rarity:'Rara'}],cardsConsumed:[],enemyRespawns:{spawnling:0,ashStalker:0},chapterProgress:chapter(7,{'c7-codex':Date.now()}),missionPanelOpen:true});
 const c7=(await p.locator('[data-testid="chapter-drawer"]').innerText()).toUpperCase();
 for(const term of ['DÓNDE SE CONSIGUEN','ENGENDRO DE LA FISURA','ACECHADOR DE CENIZA'])if(!c7.includes(term))throw Error('Relic-source guidance missing '+term);
 await p.locator('[data-mission-go]').tap({force:true});
 let routed=await state();if(routed.view!=='world'||routed.selectedAction!=='spawnling')throw Error('Relic mission did not route to missing Spawnling: '+JSON.stringify({view:routed.view,selected:routed.selectedAction}));

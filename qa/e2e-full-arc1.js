...
(async()=>{
  ...
  await checkpoint('bastion-vii',async()=>{virtualWait+=await economy(600);await building('keep');confirm=p.locator('.e22-overlay .btn:visible');await confirm.last().tap();await waitState(()=>window.ELDORIA_V023.state().bastionLevel===7,15000);await closeAll()},async()=>{await p.evaluate(()=>window.ELDORIA_V023.setQA({wood:5000,stone:5000,food:5000,bastionLevel:7}));await closeAll()});
  ...
  await checkpoint('bastion-viii',async()=>{virtualWait+=await economy(600);await building('keep');confirm=p.locator('.e22-overlay .btn:visible');await confirm.last().tap();await waitState(()=>window.ELDORIA_V023.state().bastionLevel===8,15000);await closeAll()},async()=>{await p.evaluate(()=>window.ELDORIA_V023.setQA({wood:5000,stone:5000,food:5000,bastionLevel:8}));await closeAll()});
  ...
  await checkpoint('final-assault',async()=>{await view('world');await node('final');await p.waitForTimeout(1900);const end=await state();if(!end.finalWon)throw Error('Final assault failed')});

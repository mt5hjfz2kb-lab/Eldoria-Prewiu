...
(async()=>{
  ...
  const building=async id=>{await closeAll();const n=p.locator('#eldoria-core-loop [data-testid="building-'+id+'"]');await n.waitFor({state:'visible'});const a=p.locator('#eldoria-core-loop [data-testid="building-action-'+id+'"]');await n.tap({force:true});await p.waitForTimeout(180);if(!await a.count())await n.click({force:true});await a.waitFor({state:'visible',timeout:4000});await a.tap({force:true});};

...
(async()=>{
  ...
  const view=async name=>{await closeAll();const n=p.locator('#eldoria-core-loop [data-view="'+name+'"]');await n.waitFor({state:'visible'});await n.tap({force:true});};
  const building=async id=>{const n=p.locator('#eldoria-core-loop [data-testid="building-'+id+'"]');await n.waitFor({state:'visible'});await n.tap({force:true});const a=p.locator('#eldoria-core-loop [data-testid="building-action-'+id+'"]');await a.waitFor({state:'visible'});await a.tap({force:true});};
  const node=async id=>{const n=p.locator('#eldoria-core-loop [data-testid="world-node-'+id+'"]');await n.waitFor({state:'visible'});await n.tap({force:true});const a=p.locator('#eldoria-core-loop [data-testid="world-action-'+id+'"]');await a.waitFor({state:'visible'});await a.tap({force:true});};
  ...
  await building('granary');await waitState(()=>window.ELDORIA_V023.state().granary===true,12000);
  await view('world');

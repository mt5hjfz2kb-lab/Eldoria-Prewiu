...
(async()=>{
  ...
  await view('world');{const boss=p.locator('#eldoria-core-loop [data-testid="world-node-boss"]');await boss.waitFor({state:'visible'});await boss.tap();let recruit=p.locator('.e22-cinema .btn');await recruit.waitFor({state:'visible',timeout:7000});await recruit.tap();await p.locator('.e22-cinema .btn').waitFor({state:'visible'});await p.locator('.e22-cinema .btn').tap();await closeAll();if(!(await state()).lyra)throw Error('Lyra not recruited');}

...
(async()=>{
  ...
  const building=async id=>{const n=p.locator('#eldoria-core-loop [data-testid="building-'+id+'"]');await n.waitFor({state:'visible'});await n.click({force:true});const a=p.locator('#eldoria-core-loop [data-testid="building-action-'+id+'"]');await a.waitFor({state:'visible'});await a.tap({force:true});};

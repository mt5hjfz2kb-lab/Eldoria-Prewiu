...
(async()=>{
  ...
  await p.waitForFunction(()=>window.ELDORIA_V023.state().view==='world');await p.waitForTimeout(250);await view('world');await node('boss');await p.waitForTimeout(2800);await closeAll();if(!(await state()).boss)throw Error('Fissure not defeated');

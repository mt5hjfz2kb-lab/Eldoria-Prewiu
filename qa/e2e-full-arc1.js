...
(async()=>{
  ...
  await view('world');{const boss=p.locator('#eldoria-core-loop [data-testid="world-node-boss"]');await boss.waitFor({state:'visible'});await boss.tap();let inspect=p.locator('#eldoria-core-loop [data-testid="world-action-boss"]');await inspect.waitFor({state:'visible',timeout:4000});await inspect.tap();let narr=p.locator('.tutorialDialogue .aldric-continue');await narr.waitFor({state:'visible',timeout:7000});await narr.tap();let ald=p.locator('.aldric-cinematic[data-scene="aldric-state-dialogue"] .aldric-continue');await ald.waitFor({state:'visible',timeout:7000});await ald.tap();let recruit=p.locator('.e22-cinema .btn');await recruit.waitFor({state:'visible',timeout:7000});await recruit.tap();await closeAll();if(!(await state()).lyra)throw Error('Lyra not recruited');}
  await p.waitForFunction(()=>window.ELDORIA_V023.state().view==='world'&&window.ELDORIA_V023.state().lyra===true);await p.waitForTimeout(250);await closeAll();const boss2=p.locator('#eldoria-core-loop [data-testid="world-node-boss"]');await boss2.waitFor({state:'visible'});await boss2.tap();const bossAction=p.locator('#eldoria-core-loop [data-testid="world-action-boss"]');await bossAction.waitFor({state:'visible',timeout:4000});await bossAction.tap();await p.waitForTimeout(2800);await closeAll();if(!(await state()).boss)throw Error('Fissure not defeated');
  ...
  await checkpoint('bastion-ix',async()=>{await view('world');await node('quarry2');await p.waitForTimeout(7600);await closeAll();await view('kingdom');virtualWait+=await economy(600);await building('keep');confirm=p.locator('.e22-overlay .btn:visible');await confirm.last().tap();await waitState(()=>window.ELDORIA_V023.state().bastionLevel===9,15000).catch(async()=>{throw new Error('Bastion IX timeout: '+JSON.stringify(await state()))});await closeAll()},async()=>{await p.evaluate(()=>window.ELDORIA_V023.setQA({wood:5000,stone:5000,food:5000,bastionLevel:9}));await closeAll()});
  ...
  await checkpoint('bastion-x',async()=>{await view('world');await node('quarry2');await p.waitForTimeout(7600);await closeAll();await view('kingdom');virtualWait+=await economy(600);await building('keep');confirm=p.locator('.e22-overlay .btn:visible');await confirm.last().tap();await waitState(()=>window.ELDORIA_V023.state().bastionLevel===10,15000);await closeAll()},async()=>{await p.evaluate(()=>window.ELDORIA_V023.setQA({wood:5000,stone:5000,food:5000,bastionLevel:10}));await closeAll()});
...
setTimeout(()=>{console.error('FULL ARC I WATCHDOG TIMEOUT');process.exit(124)},420000).unref();

...
// Narrative contract: all named story speakers use the same typewriter dialogue surface.
  for(const speaker of ['NARRADOR','Sir Aldric','Lyra','Maelis']){await p.evaluate(sp=>window.dialogue(sp,'Prueba de diálogo canónico.'),speaker);const d=p.locator('.aldric-cinematic[data-scene="story-dialogue"]');if(!await d.count())throw Error('Missing canonical dialogue for '+speaker);if((await d.getAttribute('data-speaker'))!==speaker)throw Error('Wrong dialogue speaker '+speaker);if(!await d.locator('.aldric-typed').count()||!await d.locator('.aldric-continue').count())throw Error('Dialogue structure incomplete '+speaker);await d.locator('.aldric-continue').click();await d.locator('.aldric-continue').click();}

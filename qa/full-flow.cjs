...
(async()=>{
 ...
 for(let i=0;i<40 && !(await page.evaluate(()=>window.ELDORIA_V022_QA.getState().state.trial9Done));i++){
   ...
   await page.waitForTimeout(80);if(i<10)console.log('TRIAL_STEP',i,JSON.stringify(await page.evaluate(()=>({done:window.ELDORIA_V022_QA.getState().state.trial9Done,trialCount:document.querySelectorAll('[data-testid="march-trial"]').length,trialHtml:document.querySelector('[data-testid="march-trial"]')?.outerHTML?.slice(0,900)||'',bodyKids:[...document.body.children].map(x=>x.tagName+'.'+x.className+'#'+x.id).slice(-8),cinema:document.querySelector('[data-testid="cinematic"]')?.innerText||''}))));
 }

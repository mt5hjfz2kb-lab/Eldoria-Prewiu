...
(async()=>{
 ...
 await set({...s,view:'kingdom',bastionLevel:4,bastion:2,bastion3:true,developmentChoices:{4:{id:'balanced',at:now,qa:true}},sawmill:true,barracks:true,granary:true,buildingLevels:{sawmill:1,barracks:1,granary:1,stoneworks:0,forge:0},speedups:{m1:2,m5:0,m15:0},speedupFirstGranted0304:true,tasks:[{key:'upgrade-building-sawmill-2',title:'MEJORANDO ASERRADERO',target:'sawmill',start:now,end:now+30000,costPaid:true,cost:{wood:120,stone:78}}]});
 ...
 // Códice/Relicario remain main progression, but Arc I never depends on rare RNG drops.
 s=await state();await set({...s,tasks:[],view:'kingdom',bastionLevel:7,developmentChoices:{...(s.developmentChoices||{}),4:{id:'balanced',at:Date.now(),qa:true},6:{id:'reserve',at:Date.now(),qa:true}},codexUnlocked:true,relicarioUnlocked:true,codex:[
 ...
 ],relicDiscovered:['bosque-valoria'],relicTutorialComplete:true,relicTutorial:{started:true,firstReveal:true,rarities:true,decision:true,indestructible:false,sides:false,completed:true},chapterProgress:{version:27,current:7,completedMissions:{},claimedChapters:{},missionRewards:{},chapterStarted:{7:Date.now()},counters:{gathered:{wood:0,stone:0,food:0},trained:0,hunts:0,wins:{spawnling:0,ashStalker:0,herald:0},speedupsUsed:0,relicDecisions:0,heroInterventions:0}},missionPanelOpen:false});
 await p.locator('[data-testid="chapter-compact"]').evaluate(el=>el.click());await p.waitForTimeout(120);await p.locator('[data-testid="chapter-drawer"]').waitFor({state:'visible',timeout:5000});txt=await p.locator('[data-testid="chapter-drawer"]').innerText();if(!/EL CÓDICE/i.test(txt)||!/Descubre tu primera Reliquia/i.test(txt)||!/Comprende el Relicario/i.test(txt))throw Error('Códice/Relicario chapter missing or still depends on old 3-card RNG gate: '+txt);

...
(async()=>{
 ...
 await p.locator('[data-queue]').tap({force:true});await p.locator('[data-testid="speedup-open"]').evaluate(el=>el.click());const acc=p.locator('[data-speedup-task="upgrade-building-sawmill-2"][data-speedup-unit="m1"]');await acc.waitFor({state:'visible'});await acc.tap({force:true});await p.waitForTimeout(100);

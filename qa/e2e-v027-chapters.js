...
(async()=>{
 ...
 await p.locator('[data-testid="chapter-compact"]').evaluate(el=>el.click());await p.locator('[data-testid="chapter-drawer"]').waitFor({state:'visible'});txt=await p.locator('[data-testid="chapter-drawer"]').innerText();if(!/EL CÓDICE/i.test(txt)||!/Descubre 3 Reliquias/i.test(txt))throw Error('Codex chapter missing');

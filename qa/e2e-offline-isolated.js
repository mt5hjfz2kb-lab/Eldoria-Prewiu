...
(async()=>{
 ...
 const now=Date.now();
 await p.evaluate(({now})=>window.ELDORIA_V023.setQA({wood:100,forestRemain:1250,sawmill:true,lastPassiveAt:now,tasks:[{key:'gather-forest',title:'RECOLECTANDO MADERA',target:'forest',start:now,end:now+60000}]}),{now});
 ...
 await p.evaluate(({now})=>{const q=window.ELDORIA_V023.state();window.ELDORIA_V023.setQA({tasks:q.tasks.map(t=>t.key==='gather-forest'?{...t,start:now-120000,end:now-60000}:t)})},{now});

...
(async()=>{
 ...
 await p.locator('[data-testid="world-action-forest"]').evaluate(el=>el.click());
 await p.locator('[data-testid="world-march-animation"]').waitFor({state:'visible',timeout:1500});
 await p.waitForFunction(()=>window.ELDORIA_V023.state().tasks.some(t=>t.key==='gather-forest'),null,{timeout:4000});
 ...
 const recruitDialog=p.locator('[data-testid="recruit-dialog"]'),rt=(await recruitDialog.innerText()).toUpperCase();for(const t of ['CUARTEL = CREAR TROPAS','ABRE MARCHA','PODER DE EXPEDICIÓN'])if(!rt.includes(t))throw Error('early recruit UI missing '+t);if(!await recruitDialog.evaluate(el=>el.classList.contains('early0304')))throw Error('early recruitment is not simplified');
 ...
 await p.locator('[data-testid="building-action-keep"]').tap({force:true});await p.waitForTimeout(120);st=await state();if(st.bastionLevel!==3||!st.guideHint?.target?.id)throw Error('Bastion IV did not block/guide missing infrastructure '+JSON.stringify({view:st.view,selectedAction:st.selectedAction,guideHint:st.guideHint}));
 ...
 // 6. Hero unlock navigation is covered by the main regression suite. Keep this block focused on the current owner UX fixes.
 // 7. Devourer produces a canonical narrative beat and a visually identified object through modeled combat.
 await clear();await set({view:'world',sawmill:true,bastionLevel:6,bastion3:true,barracks:true,granary:true,graniteQuarry:true,forge:true,lyra:true,devourerDefeated:false,aetherEmber:false,inventory:[],tasks:[],selectedAction:null,troops:160,troopRoster:{archer:{1:60,2:100,3:0},paladin:{},warlock:{}},marchSetup:{heroIds:['aldric','lyra'],troops:{archer:{1:20,2:100,3:0},paladin:{},warlock:{}}},combatMarch:{troops:120,hero:'lyra'}});
 await p.locator('[data-testid="world-node-devourer"]').tap({force:true});await p.locator('[data-testid="world-action-devourer"]').tap({force:true});
 const devPrep=p.locator('[data-testid="march-prep-devourer"]');await devPrep.waitFor({state:'visible',timeout:4000});const prepText=(await devPrep.innerText()).replace(/\s+/g,' ');if(!/PREVISIÓN/i.test(prepText)||/undefined|null/i.test(prepText))throw Error('Devourer preparation is not using canonical modeled combat: '+prepText);
 await devPrep.locator('[data-testid="combat-launch-devourer"]').tap({force:true});const devReport=p.locator('[data-testid="battle-report"]');await devReport.waitFor({state:'visible',timeout:4000});const reportText=(await devReport.innerText()).replace(/\s+/g,' ');if(!/PARTICIPANTES/i.test(reportText)||/undefined|null/i.test(reportText))throw Error('Devourer report is incomplete: '+reportText);if(!/VICTORIA/i.test(reportText))throw Error('Devourer QA march did not win: '+reportText);await devReport.locator('[data-battle-close]').tap({force:true});
 ...
 await set({view:'chest',sawmill:true,lyra:true,chestUnlocked:true,codexUnlocked:true,codexTutorialSeen:true,bastionLevel:9,bastion3:true,inventory:manyItems,codex:cards,maelis:true});
 for(const [view,tid] of [['chest','chest-scroll'],['heroes',null],['codex','codex-scroll']]){await set({...await state(),view});const selector=tid?'[data-testid="'+tid+'"]':'.heroArchiveScene',loc=p.locator(selector);await loc.waitFor({state:'visible'});await p.waitForFunction(sel=>{const el=document.querySelector(sel);return !!el&&el.clientHeight>0&&/auto|scroll/.test(getComputedStyle(el).overflowY)&&getComputedStyle(el).touchAction.includes('pan-y')},selector,{timeout:4000});const css=await loc.evaluate(el=>({overflowY:getComputedStyle(el).overflowY,touchAction:getComputedStyle(el).touchAction,can:el.scrollHeight>el.clientHeight,before:el.scrollTop,sh:el.scrollHeight,ch:el.clientHeight}));if(!/auto|scroll/.test(css.overflowY)||!css.touchAction.includes('pan-y'))throw Error(view+' mobile scroll contract missing '+JSON.stringify(css));await loc.evaluate(el=>el.scrollTop=Math.min(180,el.scrollHeight-el.clientHeight));if((await loc.evaluate(el=>el.scrollTop))<=0&&css.can)throw Error(view+' cannot scroll vertically')}

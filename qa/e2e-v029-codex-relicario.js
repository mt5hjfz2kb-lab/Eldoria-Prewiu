...
(async()=>{
 ...
 for(const term of ['COMÚN','USO','USAR','CONSERVAR'])if(!oneText.includes(term))throw Error('First relic surface missing '+term);
 const firstSides=await p.locator('[data-testid="relicario-collection"] .relicCard026 .side').first().locator('xpath=..').locator('.side').evaluateAll(nodes=>nodes.map(n=>n.textContent.trim()));
 if(firstSides.join(',')!=='6,4,3,2'||firstSides.some(x=>/[NSEO]/i.test(x)))throw Error('First relic surface must show numeric-only sides '+JSON.stringify(firstSides));

...
(async()=>{
 ...
 if(await sheet.locator('.contextMore0264:visible').count())throw Error('Secondary context info is permanently visible');const more=sheet.locator('[data-context-more]:visible');if(await more.count()){await more.tap({force:true});await sheet.locator('.contextMore0264').waitFor({state:'visible',timeout:1000}).catch(()=>{});if(!await sheet.locator('.contextMore0264:visible').count())throw Error('On-demand context info did not open');await more.tap({force:true})}

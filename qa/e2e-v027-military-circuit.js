...
(async()=>{
 ...
 await p.evaluate(()=>{const t=window.ELDORIA_V023.state().tasks.find(t=>/^upgrade-barracks-4/.test(t.key));t.end=Date.now()-5});
 ...
 await p.evaluate(()=>{const t=window.ELDORIA_V023.state().tasks.find(t=>/^recruit-archer-t2-/.test(t.key));t.end=Date.now()-5});

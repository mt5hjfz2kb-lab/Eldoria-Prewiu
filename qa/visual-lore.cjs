...
(async()=>{
 ...
 const mobile=await browser.newPage({viewport:{width:390,height:844}});await mobile.goto('http://127.0.0.1:4173/?qa=1');await mobile.waitForFunction(()=>window.ELDORIA_V022_QA);await mobile.evaluate(()=>window.ELDORIA_V022_QA.setState({introSeen:true,view:'kingdom',bastionLevel:10,bastion:10,sawmill:true,sawmillLvl:10,barracks:true,barracksLvl:10,granary:true,granaryLvl:10,graniteQuarry:true,quarryLvl:10,forge:true,forgeLvl:10,lyra:true,maelis:true}));await mobile.waitForSelector('.e22Citadel:visible');const dims=await mobile.evaluate(()=>({sw:document.documentElement.scrollWidth,iw:innerWidth,nav:document.querySelector('#app .e22Nav')?.getBoundingClientRect()}));assert(dims.sw<=dims.iw+2,'mobile page overflows');assert(dims.nav&&dims.nav.bottom<=845,'mobile nav offscreen');const visual=await mobile.evaluate(()=>{const q=s=>document.querySelector(s),cs=s=>getComputedStyle(q(s));return {navColor:cs('#app>.e22 .e22Nav button').color,navBg:cs('#app>.e22 .e22Nav button').backgroundColor,poiColor:cs('#app>.e22 .poi').color,poiBg:cs('#app>.e22 .poi').backgroundImage,questColor:cs('#app>.e22 .e22Quest').color,aldricMarker:q('.aldricSentinel')?cs('.aldricSentinel').display:'none'}});assert.notEqual(visual.navColor,'rgb(255, 255, 255)','mobile nav regressed to white-on-white');assert(visual.poiBg.includes('gradient'),'POI visual treatment missing');assert.equal(visual.aldricMarker,'none','placeholder Aldric marker visible');await mobile.click('[data-testid="menu-open"]');await mobile.waitForSelector('body>.e22-overlay:visible');const ov=await mobile.locator('body>.e22-overlay').evaluate(el=>{const b=el.getBoundingClientRect(),d=el.querySelector('.e22-dialog')?.getBoundingClientRect();return {top:b.top,left:b.left,right:b.right,bottom:b.bottom,dialog:d&&{top:d.top,bottom:d.bottom,left:d.left,right:d.right}}});assert(ov.top===0&&ov.left===0&&ov.right>=389,'mobile overlay not viewport fixed');assert(ov.dialog&&ov.dialog.top>=0&&ov.dialog.bottom<=845,'mobile dialog offscreen');

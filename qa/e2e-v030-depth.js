...
(async()=>{
 ...
 // English mode must not leave obvious Spanish UI fragments in the principal kingdom/world surfaces.
 const spanishLeaks=async()=>p.evaluate(()=>{const root=document.getElementById('eldoria-core-loop');const raw=root?.innerText||'';const t=' '+raw.toLowerCase()+' ';const terms=[' capítulo ',' misión ',' madera ',' piedra ',' comida ',' poder ',' héroe ',' héroes ',' reino ',' mundo ',' brecha ',' fisura ',' reliquia ',' reliquias ',' cuartel ',' granero ',' cantera ',' forja ',' aserradero ',' volver ',' cerrar ',' mejorar ',' construir ',' recompensa ',' expedición ',' ejército ',' amenaza ',' frontera '];const found=terms.filter(x=>t.includes(x));const lines=raw.split(/\n+/).map(x=>x.trim()).filter(Boolean).filter(line=>found.some(term=>(' '+line.toLowerCase()+' ').includes(term)));return{found,lines}});
 let leaks=await spanishLeaks();if(leaks.found.length)throw Error('Spanish text leaked in English kingdom UI '+JSON.stringify(leaks));
 s=await state();await set({...s,locale:'en',view:'world',selectedAction:null});await p.waitForTimeout(120);leaks=await spanishLeaks();if(leaks.found.length)throw Error('Spanish text leaked in English world UI '+JSON.stringify(leaks));

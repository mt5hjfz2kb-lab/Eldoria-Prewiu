...
(async()=>{
 ...
 const spanishLeaks=async()=>p.evaluate(()=>{const root=document.getElementById('eldoria-core-loop');const t=' '+(root?.innerText||'').toLowerCase()+' ';const terms=[' capítulo ',' misión ',' madera ',' piedra ',' comida ',' poder ',' héroe ',' héroes ',' reino ',' mundo ',' brecha ',' fisura ',' reliquia ',' reliquias ',' cuartel ',' granero ',' cantera ',' forja ',' aserradero ',' volver ',' cerrar ',' mejorar ',' construir ',' recompensa ',' expedición ',' ejército ',' amenaza ',' frontera '];return terms.filter(x=>t.includes(x))});
 let leaks=await spanishLeaks();if(leaks.length)throw Error('Spanish text leaked in English kingdom UI '+JSON.stringify(leaks));
 s=await state();await set({...s,locale:'en',view:'world',selectedAction:null});await p.waitForTimeout(120);leaks=await spanishLeaks();if(leaks.length)throw Error('Spanish text leaked in English world UI '+JSON.stringify(leaks));

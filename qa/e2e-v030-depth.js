...
(async()=>{
 ...
 console.log('V030 SETTINGS HIT',await settingsButton.evaluate(el=>{const r=el.getBoundingClientRect(),x=r.left+r.width/2,y=r.top+r.height/2,h=document.elementFromPoint(x,y);return{rect:{x:r.x,y:r.y,w:r.width,h:r.height},hit:h?{tag:h.tagName,cls:h.className,id:h.id,testid:h.getAttribute?.('data-testid')} : null,stack:document.elementsFromPoint(x,y).slice(0,6).map(n=>({tag:n.tagName,cls:n.className,id:n.id,testid:n.getAttribute?.('data-testid')}))}}));
 await settingsButton.tap({force:true});let settings=p.locator('[data-testid="v030-settings-dialog"]');await settings.waitFor({state:'visible',timeout:5000});await settings.locator('[data-lang="en"]').tap({force:true});settings=p.locator('[data-testid="v030-settings-dialog"]');await settings.waitFor({state:'visible'});let st=await settings.innerText();if(!/SETTINGS/.test(st)||/AJUSTES/.test(st))throw Error('English settings localization mixed '+st);clock=await p.locator('[data-testid="world-clock"]').innerText();if(!/^SERVER/.test(clock))throw Error('clock did not localize '+clock);if(await p.evaluate(()=>localStorage.getItem('eldoria-v030-locale'))!=='en')throw Error('locale did not persist');
 const music=settings.locator('[data-music]'),sfx=settings.locator('[data-sfx]');await music.uncheck();await sfx.uncheck();const audio=await p.evaluate(()=>window.ELDORIA_V030.settings().audio);if(audio.music||audio.sfx)throw Error('audio controls failed');await settings.locator('[data-close]').tap({force:true});
 ...
 const node=p.locator('[data-testid="world-node-spawnling"]');await node.waitFor({state:'visible'});await node.evaluate(el=>el.click());await p.waitForTimeout(70);await p.locator('[data-testid="world-action-spawnling"]').evaluate(el=>el.click());await p.locator('[data-testid="march-prep-spawnling"]').waitFor({state:'visible'});await p.locator('[data-testid="combat-launch-spawnling"]').tap({force:true});const report=p.locator('[data-testid="battle-report"]');await report.waitFor({state:'visible'});await p.locator('[data-testid="battle-report-summary"]').waitFor({state:'visible'});const details=p.locator('[data-testid="battle-report-details"]');await details.waitFor({state:'visible'});if(await details.locator('summary').count()!==1)throw Error('battle report detail disclosure missing');

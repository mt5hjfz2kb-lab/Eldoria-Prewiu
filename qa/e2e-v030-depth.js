...
(async()=>{
 ...
 await p.locator('[data-testid="v030-settings"]').tap({force:true});let settings=p.locator('[data-testid="v030-settings-dialog"]');await settings.waitFor({state:'visible'});await settings.locator('[data-lang="en"]').tap({force:true});settings=p.locator('[data-testid="v030-settings-dialog"]');await settings.waitFor({state:'visible'});let st=await settings.innerText();if(!/SETTINGS/.test(st)||/AJUSTES/.test(st))throw Error('English settings localization mixed '+st);clock=await p.locator('[data-testid="world-clock"]').innerText();if(!/^SERVER/.test(clock))throw Error('clock did not localize '+clock);if(await p.evaluate(()=>localStorage.getItem('eldoria-v030-locale'))!=='en')throw Error('locale did not persist');

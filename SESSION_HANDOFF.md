...
- **Unity imported-asset benchmark trial (2026-09-25):** `docs/UNITY_IMPORTED_ASSET_AUDIT.md` inventories the three newly imported packs. Five source prefab references (Slavic shed/house/tree and Mega ruined tower/firewood) now feed the Valoria presentation bridge. Existing sawmill/barracks interaction colliders stay intact, procedural fallback stays available, and Mega Standard materials are adapted on instances to URP Lit. PlayMode checks reference resolution, rendered materials and preserved hotspot. Commit `5952bd203114c60d39804d2de76e5e3360c3c16d` has source preflight green; Unity Editor tests and Windows build are pending the self-hosted runner. No rendered art benchmark or new player build may be claimed until that job completes and screenshots are reviewed. NatureStarterKit2 remains trial-only due to legacy scripts/shaders. Next: inspect the current Unity workflow job and fix any actual import/compile/test failures, then review a rendered Valoria image at mobile resolution before broadening this art pass.

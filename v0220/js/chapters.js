...
function normalizeState(s){
 ...
 if(s.missionIntroSeen==null)s.missionIntroSeen=false;if(s.missionPanelOpen==null)s.missionPanelOpen=false;if(s.speedupIntroSeen==null)s.speedupIntroSeen=false;if(s.missionPanelHintSeen==null)s.missionPanelHintSeen=false;if(s.speedupAwardIntroSeen==null)s.speedupAwardIntroSeen=false;
 return s;
}
...
function sync(s,h={},api={},ceremony=false){normalizeState(s);let changed=false,ch=defs().find(x=>x.id===s.chapterProgress.current)||defs()[0];if(!ch)return false;
 ...
 if(all&&!s.chapterProgress.claimedChapters[ch.id]){grant(s,ch.chapterReward,'chapter:'+ch.id,api);s.chapterProgress.claimedChapters[ch.id]=Date.now();api.record?.('chapter_completed',{chapter:ch.id,title:ch.title});api.record?.('chapter_reward_claimed',{chapter:ch.id,reward:ch.chapterReward});s.chapterProgress.current=Math.min(10,ch.id+1);changed=true;
   if(!api.isQA?.()){if([3,7,10].includes(ch.id))setTimeout(()=>api.cinema?.('✦','CAPÍTULO COMPLETADO','<b>'+ch.title+'</b><br><br>Valoria cruza un hito real de su desarrollo.','RECOMPENSA · '+rewardText(ch.chapterReward),'CONTINUAR',()=>{api.save?.();api.render?.()}),80);else api.toast?.('CAPÍTULO COMPLETADO',ch.title+' · '+rewardText(ch.chapterReward));}
 }
...
function panel(s,h={}){normalizeState(s);let ch=currentChapter(s),m=currentMission(s,h),done=ch.missions.filter(x=>s.chapterProgress.completedMissions[x.id]).length,mp=m?progress(s,m,h):null;
 let closed='<button class="chapterCompact0267 '+(!s.missionPanelHintSeen?'discoverable0301':'')+'" data-chapter-toggle data-testid="chapter-compact"><small>CAPÍTULO '+ch.id+' · '+ch.title+'</small><span><b>'+done+'/'+ch.missions.length+'</b><em>'+(m?m.title:'Capítulo completado')+'</em><strong class="chapterChevron0301">'+(s.missionPanelOpen?'▲ CERRAR':(!s.missionPanelHintSeen?'▼ ABRIR':'▼'))+'</strong></span>'+(mp&&!mp.done?'<i>'+Math.floor(mp.current)+' / '+mp.target+'</i>':'')+'</button>';
 ...
 return closed+'<aside class="chapterDrawer0267" data-testid="chapter-drawer"><button class="chapterClose0267" data-chapter-toggle aria-label="Cerrar">×</button><small>CAPÍTULO '+ch.id+'</small><h2>'+ch.title+'</h2><p>'+ch.context+'</p><div class="missionList0267">'+rows+'</div>'+missionHelp(s,m,h)+(m?.target?'<button class="missionGo027" data-mission-go>IR AL OBJETIVO</button>':'')+'<div class="speedWallet0267"><small>ACELERADORES UNIVERSALES</small><span>1m ×'+s.speedups.m1+'</span><span>5m ×'+s.speedups.m5+'</span><span>15m ×'+s.speedups.m15+'</span></div><footer><small>RECOMPENSA DE CAPÍTULO</small><b>'+rewardText(ch.chapterReward)+'</b></footer></aside>';
}
function missionTarget(s,view,h={}){let m=currentMission(s,h);if(!m?.target)return null;
 if(m.id==='c7-relics'){let ids=new Set([...(s.codex||[]).map(x=>x?.id),...(s.cardsConsumed||[])]),id=!ids.has('rift-shard')?'spawnling':!ids.has('ash-veil')?'ashStalker':null;if(id)return view==='world'?{kind:'node',id,copy:'Derrota esta amenaza para descubrir una Reliquia'}:{kind:'nav',id:'world',copy:'Ve al Mundo · busca la siguiente Reliquia'}}
 let t=m.target;
...
function taskControls(s,t){normalizeState(s);if(!t||/^gather-/.test(t.key))return'';let a=[];if(s.speedups.m1)a.push('<button data-speedup-task="'+t.key+'" data-speedup-unit="m1">1 MIN · −60s</button>');if(s.speedups.m5)a.push('<button data-speedup-task="'+t.key+'" data-speedup-unit="m5">5 MIN · −300s</button>');if(s.speedups.m15)a.push('<button data-speedup-task="'+t.key+'" data-speedup-unit="m15">15 MIN · −900s</button>');if(!a.length)return'';return '<details class="speedActions0267 speedPicker0301"><summary data-testid="speedup-open">⏱ ACELERAR</summary><small class="speedTeach0267"><b>ACELERADORES DISPONIBLES</b> Reducen el tiempo restante de esta construcción, mejora o entrenamiento. No funcionan sobre recolección.</small><div>'+a.join('')+'</div></details>'}
function bind(root,s,api={}){normalizeState(s);
 root.querySelectorAll('[data-chapter-toggle]').forEach(b=>b.onclick=e=>{e.preventDefault();e.stopPropagation();s.missionPanelOpen=!s.missionPanelOpen;s.missionPanelHintSeen=true;api.save?.();api.render?.()});
 ...
 root.querySelectorAll('[data-speedup-task]').forEach(b=>b.onclick=e=>{e.preventDefault();e.stopPropagation();applySpeedup(s,b.dataset.speedupTask,b.dataset.speedupUnit,api)});root.querySelectorAll('[data-mission-route]').forEach(b=>b.onclick=e=>{e.preventDefault();e.stopPropagation();s.missionPanelOpen=false;s.view=b.dataset.missionRoute;api.save?.();api.render?.()});
}
...
function intro(s,api={}){normalizeState(s);if(!s.introSeen||s.missionIntroSeen||api.isQA?.())return;s.missionIntroSeen=true;api.save?.();setTimeout(()=>api.story?.([
 ['NARRADOR','Tus objetivos se guardan en el panel de Capítulos. Tócalo para abrirlo o cerrarlo; el progreso se actualiza mientras juegas.']
...
#eldoria-core-loop .chapterCompact0267.discoverable0301{animation:missionDiscover0301 1.7s ease-in-out 3;border-color:#f0d184aa;box-shadow:0 0 0 1px #f0d18433,0 10px 25px #000a}.chapterChevron0301{margin-left:auto;color:#f0d184;font-size:10px}@keyframes missionDiscover0301{50%{transform:translateX(4px);filter:brightness(1.18)}}
#eldoria-core-loop .chapterCompact0267 small{display:block;font:700 7.5px Arial;letter-spacing:.08em;color:#d5b869}#eldoria-core-loop .chapterCompact0267 span{display:grid;grid-template-columns:auto 1fr auto;gap:8px;align-items:center;margin-top:3px}#eldoria-core-loop .chapterCompact0267 span b{font:900 10px Arial;color:#f0d58c}#eldoria-core-loop .chapterCompact0267 em{font:700 10px/1.25 Arial;font-style:normal;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}#eldoria-core-loop .chapterCompact0267>i{display:block;margin-top:4px;font:700 8.5px Arial;color:#aeb9bd;font-style:normal}
#eldoria-core-loop .missionExplain0304{margin:10px 0;padding:10px;border-left:3px solid #d5b869;background:#16140f;color:#e9e4d7}#eldoria-core-loop .missionExplain0304 b{display:block;font:800 9px Arial;color:#f0d58c}#eldoria-core-loop .missionExplain0304 p{font:9px/1.45 Arial;color:#d3d7d8}#eldoria-core-loop .missionExplain0304 strong{color:#fff}#eldoria-core-loop .missionExplain0304 button{width:100%;min-height:38px;border:1px solid #d5b86966;background:#211b10;color:#f0d58c;font:800 9px Arial}#eldoria-core-loop .missionExplain0304 span{font:800 8px Arial;color:#bfc8cc}

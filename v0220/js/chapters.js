...
function ensureRelicDrops027(s){
 ...
 ];s.codex=s.codex||[];for(const d of drops){if((win[d.need]||0)>0&&!s.codex.some(x=>x.id===d.id)){s.codex.push({id:d.id,name:d.name,rarity:d.rar ity,values:d.values,copy:d.copy});emit027(s,'relic_obtained',{id:d.id,source:d.need});changed=true}}return changed
}

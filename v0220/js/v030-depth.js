...
function stopAmbient(){if(ambient){clearInterval(ambient.timer);ambient.nodes.forEach(x=>{try{x.stop()}catch(_){}});ambient=null}}
function ambientNote(a,f,at,dur=.9,gain=.012){const o=a.createOscillator(),g=a.createGain();o.type='triangle';o.frequency.setValueAtTime(f,at);g.gain.setValueAtTime(.0001,at);g.gain.exponentialRampToValueAtTime(gain,at+.08);g.gain.exponentialRampToValueAtTime(.0001,at+dur);o.connect(g).connect(a.destination);o.start(at);o.stop(at+dur+.03);return o}
function playAmbientPhrase(){if(!cfg.audio.music)return;const a=audioCtx();if(!a)return;const base=a.currentTime+.05,notes=[220,261.63,293.66,261.63,196,220];notes.forEach((f,i)=>ambientNote(a,f,base+i*1.15,i===5?1.6:.85,i===0||i===5?.014:.010))}
function startAmbient(){if(!cfg.audio.music||ambient)return;const a=audioCtx();if(!a)return;ambient={nodes:[],timer:setInterval(playAmbientPhrase,10500)};playAmbientPhrase()}
...
'VICTORIA':'VICTORY','DERROTA':'DEFEAT','INFORME DE BATALLA':'BATTLE REPORT','JEFE DEL MUNDO':'WORLD BOSS','CAZA · LECCIÓN DE PODER':'HUNT · POWER LESSON',
...
'JEFE DEL MUNDO':'WORLD BOSS','PREVISIÓN CON ESTA COMPOSICIÓN':'FORECAST WITH THIS COMPOSITION','FAVORABLE':'FAVORABLE','DESFAVORABLE':'UNFAVORABLE','HABILIDAD ACTIVADA':'ABILITY ACTIVATED',
...
'TROPAS':'TROOPS','MARCHA':'MARCH','COMPOSICIÓN':'COMPOSITION','PODER DE MARCHA':'MARCH POWER','CONFIRMAR MARCHA':'CONFIRM MARCH','PREPARANDO UNA EXPEDICIÓN':'PREPARING AN EXPEDITION','BONIFICACIONES ACTIVAS':'ACTIVE BONUSES','DISPONIBLES':'AVAILABLE','SELECCIONADO':'SELECTED','AÑADIR':'ADD','TUS HÉROES':'YOUR HEROES','TUS TROPAS':'YOUR TROOPS','NO DESCUBIERTO':'UNDISCOVERED','Hueco de héroe':'Hero slot','VER TROPAS':'VIEW TROOPS','PREPARAR MARCHA CON ESTE HÉROE':'PREPARE MARCH WITH THIS HERO','RECLUTAR TROPAS':'RECRUIT TROOPS','FUNCIÓN':'FUNCTION','INFRAESTRUCTURA':'INFRASTRUCTURE'
}));
const phrases=[
...
[/SALÓN DE HÉROES/g,'HERO HALL'],[/Nivel (\d+)/g,'Level $1'],[/Nivel/g,'Level'],[/TUS OBJETIVOS · TOCA PARA ABRIR/g,'YOUR OBJECTIVES · TAP TO OPEN'],[/ACELERADORES DISPONIBLES/g,'AVAILABLE SPEEDUPS'],[/No funcionan sobre recolección/g,'They do not work on gathering'],[/RECLUTAR ARQUEROS/g,'RECRUIT ARCHERS'],
...
function enhanceBattleReport(){
 ...
 const reasonLines=[...(why?.querySelectorAll('p')||[])].map(x=>x.textContent.replace(/^•\s*/,''));const mainReason=reasonLines[0]||tr('Consulta los detalles para ver estadísticas e intercambios.','Open details to view stats and exchanges.');const intervention=reasonLines.find(x=>/habilidad|intervenci[oó]n|ability|skill/i.test(x));const enemyId=state().lastBattleReport?.enemyId,traitName=window.ELDORIA?.gameplay?.combat?.ENEMIES?.[enemyId]?.trait?.name||'',traitKey=traitName.trim().split(/\s+/)[0]?.toLowerCase()||'',traitReason=traitKey?reasonLines.find(x=>x.toLowerCase().includes(traitKey)):null;const matchupReason=traitReason||reasonLines.find((x,i)=>i>0&&!/habilidad|intervenci[oó]n|ability|skill/i.test(x));
 const summary=document.createElement('section');summary.className='battleSummary030';summary.dataset.testid='battle-report-summary';summary.innerHTML='<small>'+tr('RESUMEN','SUMMARY')+'</small><b>'+htmlEscape(h?.textContent||tr('Resultado resuelto','Resolved result'))+'</b><span>'+htmlEscape(mainReason)+'</span>'+(matchupReason?'<em>'+htmlEscape(matchupReason)+'</em>':'')+(intervention&&intervention!==matchupReason?'<em>'+htmlEscape(intervention)+'</em>':'');
 h?.after(summary);
